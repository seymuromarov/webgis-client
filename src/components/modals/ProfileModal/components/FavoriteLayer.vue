<template>
  <div v-if="basemaps.length > 0 || dynamics.length > 0" class="row">
    ...
    <div class="col-md-12">
      <button
        type="button"
        class="btn btn-success mt-5 float-right btn-sm"
        @click="submit"
      >
...
export default {
  ...
  mounted() {
    this.getFavoriteLayers().then((favoriteLayers) => {
      ...
      console.log("mounted -> this.checkedDynamics", this.checkedDynamics);
      let checkedBasemaps = favoriteLayers
        .filter((c) => c.layer.mapTypeId == "basemap")
        .map((c) => c.layer.id);
      this.checkedBasemaps = checkedBasemaps;
      console.log("mounted ->  this.checkedBasemaps", this.checkedBasemaps);
