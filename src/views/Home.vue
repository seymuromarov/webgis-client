...
export default {
    ...
    methods: {
        ...
        filterSelectedColumn(id, column) {
            ...
            if (serviceHelper.isLocalService(service)) {
                LayerService.getLayerColumnsDistinctData(params)
                    .then(response => {
                    ...
                    .catch(error => {
                        this.$snotify("Something went wrong");
                    });
            } else {
        ...
        async getTableData(service, layerId, layerName, query) {
            ...
            if (serviceHelper.isArcgisService(service)) {
                ...
                if (isBunch) {
                    ...
                    if (
                        this.filterQueryIsSum &&
                        service.resourceType === "local"
                    ) {
        ...
        isLayerColorEnabled(service) {
            let isColorEnabled = false;
            LayerService.getLayerDynamic({
                token: this.token,
                name: service.name,
            })
                .then(response => {
                    if (response.data.layers[0].drawingInfo !== undefined) {
                        if (
                            response.data.layers[0].drawingInfo.renderer
                                .symbol !== undefined
                        ) {
                            if (
                                response.data.layers[0].drawingInfo.renderer
        ...
        getResponseDynamic(service) {
            if (serviceHelper.isDynamicFromArcgis(service)) {
                LayerService.getDynamicLayers({
                    token: this.token,
                    name: service.name,
                })
                    .then(response => {
