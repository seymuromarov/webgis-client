<template>
    <div class="row container-fluid padding-0">
        ...
        <!-- Change Detection -->
        <detector-modal :visible="bbox.length > 0 && isDrawnShapeForDetection"
                        v-bind="{ bbox, token }"
                        @close="(bbox = []) & (isDrawnShapeForDetection = false)"></detector-modal>
        <!-- Information Modal -->
        <InfoModal :isOpen="isInformationModalVisible"
        ...
        mounted() {
            ...
            this.$nextTick(function () {
                ...
                this.mapLayer.on("click", (evt) => {
                    ...
                    if (self.isRemove) {
                        self.mapLayer.forEachFeatureAtPixel(evt.pixel, (
                            feature,
                            layer
                        ) => {
                            try {
                                ...
                                let elem = document.getElementsByClassName(`feature-${feature.get("id")}`);
                                elem[0].remove();
